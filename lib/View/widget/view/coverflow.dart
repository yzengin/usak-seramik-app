        ...
        builder: (context, constraints) {
          ...
          return GestureDetector(
            onTapUp: (detail) {
              ...
              final double centerLeftEdge = centerPosition - centerWidgetWidth / 2;
              final double centerRightEdge = centerPosition + centerWidgetWidth / 2;

              if (detail.localPosition.dx <= centerLeftEdge) {
                _pageController!.animateToPage(_currentPagePosition.toInt() - 1, duration: Duration(milliseconds: 500), curve: Curves.easeIn);
...
class CoverFlowCardItems extends StatelessWidget {
  ...
  double getCardPosition(int index) {
    ...
    if (distance == 0) {
      ...
      if (distance > 0) {
        ...
        return (basePosition + centerWidgetWidth + nearWidgetWidth) + farWidgetWidth * (distance.abs() - 2) - (nearWidgetWidth - farWidgetWidth) * ((distance - distance.floor()));
      }
    } else {
      if (distance > 0) {
        return (basePosition - nearWidgetWidth) - farWidgetWidth * (distance.abs() - 1);
      } else {
        return (basePosition + centerWidgetWidth + nearWidgetWidth) + farWidgetWidth * (distance.abs() - 2);
      }
    }
  }
  ...
  double getCardWidth(int index) {
    ...
    if (distance >= 0.0 && distance < 1.0) {
      return centerWidgetWidth - (centerWidgetWidth - nearWidgetWidth) * (distance - distance.floor());
    } else if (distance >= 1.0 && distance < 2.0) {
      return nearWidgetWidth - (nearWidgetWidth - farWidgetWidth) * (distance - distance.floor());
    } else {
    ...
    return Center(
      child: Stack(
        ...
        children: <Widget>[
          ...
          if (titles != null && displayOnlyCenterTitle!) _buildTitle(currentPagePosition.toInt()),
        ],
      ),
    );
  }
